---
title: 为Forms as a Cloud Service沙盒设置开发环境
...
<span class="preview">此文章是&#x200B;**正在处理的工作**。</span>


准备好在Cloud Service上构建和测试Headless自适应表单了吗？ 为Cloud Service项目启用Forms并开始。
...
* 为您的Cloud Service项目[&#128279;](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-cloud-service/content/release-notes/prerelease#cloud-environments)启用预发行版渠道。

## 设置工作流

要在Forms as a Cloud Service沙盒上启用Headless自适应表单，请为您的AEM Cloud Service项目启用`Forms - Digital enrolment`解决方案。 然后，在本地计算机上创建一个基于原型37或更高版本的项目，并将其推送到您的Forms as a Cloud Service环境。 完整的过程是：
...
>[!INFO]
>
> 您始终可以提交到Git存储库而不对其进行克隆。 但是，它也有自己的怪癖。 因此，本文档将使用克隆方法。
...
现在，您的环境已准备好使用Headless自适应表单。 您现在可以将表单的JSON定义上传到Cloud Service环境。 然后，基于它创建Headless自适应表单，并使用[getForm](https://opensource.adobe.com/aem-forms-af-runtime/api/#tag/Get-Form-Definition/operation/getForm)和其他rest API在应用程序或服务中使用Headless自适应表单。
